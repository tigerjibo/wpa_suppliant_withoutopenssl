...
#endif
        {
          struct wpa_ptk *wptk = (struct wpa_ptk *)ptk;
          wpa_eapol_key_send(sm, wptk->kck, ver, dst, ETH_P_EAPOL,
                               rbuf, rlen, reply->key_mic);
        }
        return 0;
}
                                          ...
                                          u16 ver)
{
        ...
        /* PMK - Get the PMK from the EAP state machine */
        /* PMK in case of LEAP is 16 bytes
         * PMK in other cases is 32 bytes 
         */
                                      ...
                                      const struct wpa_eapol_key *key)
{
        ...
        if (sm->key_mgmt == WPA_KEY_MGMT_CCKM)
        {
           /* Parameters are as follows - alg: CCKM  KIP
            * BSSID of the AP 
                                          ...
                                          u16 ver)
{
        ...
        if (connected)
        {
          ...
          /* First flush the keys and then send 4/4 and re-install the keys */
          if (wpa_sm_set_key(sm, alg, sm->bssid, 0, 1, NULL, 0,
                             NULL, 0) < 0) {
                goto failed;
          }
        }
                                      ...
                                      int ver, u16 key_info)
{
        ...
        wpa_eapol_key_send(sm, kck, ver, sm->bssid, ETH_P_EAPOL,
                           rbuf, rlen, reply->key_mic);
                                       ...
                                       u8 *gd)
{
        ...
        buff[7] = 28;
        buff[24] = 2;
        u8 auth_oui[] = {0x0, 0x50, 0xf2, 0x01, 0x28, 0x00};
        os_memcpy(&buff[length], auth_oui, 6); 
        length += 6;
#endif
        /* Rekey number */
        ...
        wpa_eapol_key_mic(sm->cptk.kck, 1, buff, length, mic_new);
        if (os_memcmp(mic, mic_new, 8) != 0) {
                ...
                os_memset(gtk_data->gtk, 0, sizeof(struct wpa_gtk_data));
                ...
                wpa_supplicant_check_group_cipher(sm->group_cipher,
                                                  *gkey_len, *gkey_len,
                                           ...
                                           struct wpa_eapol_key *key, u16 ver)
{
        ...
        if (ver == WPA_KEY_INFO_TYPE_HMAC_MD5_RC4) {
                ...
                if (aes_unwrap(kek, keydatalen / 8,
                               (u8 *) (key + 1), buf)) {
                        os_free(buf);
